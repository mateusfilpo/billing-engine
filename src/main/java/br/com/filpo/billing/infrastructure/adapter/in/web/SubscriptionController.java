...
import br.com.filpo.billing.infrastructure.adapter.in.web.dto.ChangePlanRequest;
...
public class SubscriptionController {
    ...
    private final SubscriptionWebMapper subscriptionMapper;
    private final InvoiceWebMapper invoiceMapper;
    ...
    public ResponseEntity<SubscriptionResponse> create(@Valid @RequestBody CreateSubscriptionRequest request) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(subscriptionMapper.toResponse(savedSubscription));
    }
    ...
    public ResponseEntity<List<SubscriptionResponse>> findByCustomerId(@RequestParam UUID customerId) {
        ...
        return ResponseEntity.ok(subscriptionMapper.toResponseList(subscriptions));
    }
    ...
    public ResponseEntity<SubscriptionResponse> cancel(@PathVariable UUID id) {
        ...
        return ResponseEntity.ok(subscriptionMapper.toResponse(cancelledSubscription));
    }

    @PostMapping("/{id}/change-plan")
    public ResponseEntity<InvoiceResponse> changePlan(@PathVariable UUID id,
            @Valid @RequestBody ChangePlanRequest request) {
        Invoice adjustmentInvoice = changePlanUseCase.changePlan(id, request.newPlanId());
        return ResponseEntity.ok(invoiceMapper.toResponse(adjustmentInvoice));
    }
}